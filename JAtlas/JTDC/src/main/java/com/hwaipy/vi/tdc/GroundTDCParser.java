...
public class GroundTDCParser {
  ...
  public long[][] offer(byte[] data) {
    ...
    parsedDataCount += dataBuffer.position();
    dataBuffer.compact();
    long[][] te = new long[timeEvents.size()][2];
    Iterator<long[]> iterator = timeEvents.iterator();
    for (int i = 0; i < timeEvents.size(); i++) {
      te[i] = iterator.next();
    }
    return te;
  }
  private long parsedDataCount = 0;
  ...
  private long[] parseToTimeEvent(int position) {
